...
from dontmanage.integrations.utils import make_post_request, make_request
...
class WhatsAppTemplates(Document):
    ...
    def after_insert(self):
        ...
        self.language_code = dontmanage.db.get_value(
        ...
        try:
            ...
            # dontmanage.db.set_value("WhatsApp Templates", self.name, "id", response['id'])
    ...
    def get_settings(self):
        ...
        settings = dontmanage.get_doc("WhatsApp Settings", "WhatsApp Settings")
    ...
    def on_trash(self):
        ...
        try:
            ...
            if res.get("error_user_title") == "Message Template Not Found":
                dontmanage.msgprint("Deleted locally", res.get("error_user_title", "Error"), alert=True)
            else:
                dontmanage.throw(
    ...
    def get_header(self):
        ...
        if self.header_type == "TEXT":
            ...
            if not self.sample:
                key = dontmanage.get_doc(self.doctype, self.name).get_document_share_key()
                link = get_pdf_link(self.doctype, self.name)
                self.sample = f'{dontmanage.utils.get_url()}{link}&key={key}'
...
def fetch():
    ...
    try:
        ...
        for template in response['data']:
            ...
            flags = 1
            if dontmanage.db.exists("WhatsApp Templates", template['name']):
                doc = dontmanage.get_doc("WhatsApp Templates", template['name'])
            else:
                flags = 0
                doc = dontmanage.new_doc("WhatsApp Templates")
