...
class WhatsAppNotification(Document):
    ...
    def validate(self):
        ...
        if self.notification_type == "DocType Event":
            fields = dontmanage.get_doc("DocType", self.reference_doctype).fields
            fields += dontmanage.get_all(
                "Custom Field",
                filters={"dt": self.reference_doctype},
                fields=["fieldname"]
            )
            if not any(field.fieldname == self.field_name for field in fields): # noqa
                dontmanage.throw(f"Field name {self.field_name} does not exists")
    ...
    def send_scheduled_message(self) -> dict:
        ...
        language_code = dontmanage.db.get_value(
    ...
    def send_template_message(self, doc: Document):
        ...
        if template:
            ...
            if self.attach_document_print:
                ...
                doctype = dontmanage.get_doc("DocType", doc_data['doctype'])
                if doctype.custom:
                ...
                url = f'{dontmanage.utils.get_url()}{link}&key={key}'
                ...
                if self.attach.startswith("http"):
                    ...
                    url = f'{dontmanage.utils.get_url()}{self.attach}'
    ...
    def notify(self, data):
        ...
        try:
            ...
            dontmanage.msgprint("WhatsApp Message Triggered", indicator="green", alert=True)

        except Exception as e:
            response = dontmanage.flags.integration_request.json()['error']
    ...
    def on_trash(self):
        ...
        if self.notification_type == "Scheduler Event":
            dontmanage.delete_doc("Scheduled Job Type", self.name)

        dontmanage.cache().delete_value("whatsapp_notification_map")

    def after_insert(self):
        """After insert hook."""
        if self.notification_type == "Scheduler Event":
            method = f"dontmanage_whatsapp.utils.trigger_whatsapp_notifications_{self.event_frequency.lower().replace(' ', '_')}" # noqa
